            ...
            if (Request.Form["Key"] != null)
            {
                ...
                else if (Key == "register")
                {
                    try
                    {
                        ...
                        sql = "select * from [userTable] where userName='" + usrName + "' or eMail='"+eMail+"'";
                        SqlCommand cmd = new SqlCommand(sql, connection);
                        SqlDataReader reader = cmd.ExecuteReader();
                        DataTable dataTable = new DataTable();
                        dataTable.Load(reader);
                        ...
                        {
                            reader.Close();
                            sql = "insert into [userTable]([userName],[userPassword],[registerTime],[eMail],[userType],[userStatution])values('" + usrName + "','" + usrPassword + "','" + registerTime + "','" + eMail + "','U0','D1')";
